...
local function compute_values()
	...
	values["LUA_HEAP (collectgarbage)"] = collectgarbage("count") * 1024
...
mem_profile.track = function(name, value)
	...
	-- Never append a nil value: tracked[name] would stay nil and the name would be
	-- re-appended to order every frame (unbounded growth of the profiler itself).
	if value == nil then
		return
	end
